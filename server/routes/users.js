...
// ─────────────────────────────────────────────────────────────
// @desc    Lấy danh sách toàn bộ users
// @route   GET /api/users
...
// @access  Admin, Moderator (hoặc chính user đó)
// ─────────────────────────────────────────────────────────────
router.get('/:id', protect, async (req, res) => {
    try {
        ...
        if (!isSelf && !isPrivileged) {
            return res.status(403).json({ success: false, message: 'Không có quyền xem thông tin người dùng khác' });
        }

        const user = await User.findById(req.params.id)
            .select('-password')
            .populate({ path: 'postedQuestions', select: 'title tags viewCount votes createdAt', options: { limit: 10, sort: { createdAt: -1 } } })
            .populate({ path: 'postedAnswers', select: 'content votes isAccepted question createdAt', options: { limit: 10, sort: { createdAt: -1 } } });
...
router.put('/:id/role', protect, authorize('admin'), async (req, res) => {
    try {
        ...
        // Không cho phép hạ role của chính mình
...
router.put('/:id/toggle-active', protect, authorize('admin', 'moderator'), async (req, res) => {
    try {
        ...
        // Moderator không được khoá Admin
...
// ─────────────────────────────────────────────────────────────
// @desc    Xoá user (xoá mềm = khoá vĩnh viễn, hoặc xoá cứng)
...
// ─────────────────────────────────────────────────────────────
// @desc    Thống kê user theo role / khoa
